...
/**
 * 共通なFilterのモデルから、Sql ServerのWhere句を作成する
 *
 * @author Legend Applications China, LaKeel BI development team.
 * @since 2018/1/31
 */
public class SqlserverFilterSQLImpl extends GenericFilterSQL {
	...
	private static final String timeStampEqualPattern = "F = convert(varchar(24),'A',120)";
	private static final String timeStampEqualNotEqualPattern = "(F != convert(varchar(24),'A',120) or F is null)";
	private static final String timeStampBeforePattern = "F < convert(varchar(24),'A',120)";
	private static final String timeStampAfterPattern = "F > convert(varchar(24),'A',120)";
	private static final String timeStampBetweenPattern = "F between convert(varchar(24),'A',120) and convert(varchar(24),'B',120)";
	private static final String timeStampNotBetweenPattern = "((F not between convert(varchar(24),'A',120) and convert(varchar(24),'B',120)) or F is null)";
	private static final String timeStampBeforeOnPattern = "F <= convert(varchar(24),'A',120)";
	private static final String timeStampAfterOnPattern = "F >= convert(varchar(24),'A',120)";
	...
	private static SqlserverFilterSQLImpl instance;

	public static SqlserverFilterSQLImpl getInstance() {
		if (instance == null) {
			instance = new SqlserverFilterSQLImpl();
		}
		return instance;
	}

	private SqlserverFilterSQLImpl() {
		super();
		...
		bitEqual = filter -> {
			if (filter.getValue().toString().equals("true")) {
				return EqualPattern.replace(fieldId, SQLUtils.DBsurround(filter.getFieldId().toLowerCase()))
						.replace(numberValue, "1");
			} else if (filter.getValue().toString().equals("false")) {
				return EqualPattern.replace(fieldId, SQLUtils.DBsurround(filter.getFieldId().toLowerCase()))
						.replace(numberValue, "0");
			} else {
	...
	private String repalceLikeSql(Filter filter, String sql) {
		...
		if (varcharValue.contains("%") || varcharValue.contains("_")) {
			...
			return sql.replace(escape, varcharLikeEscape).replace(fieldId, SQLUtils.DBsurround(filter.getFieldId().toLowerCase())).replace(value,
					varcharValue);
		}
		return sql.replace(escape, "").replace(fieldId, SQLUtils.DBsurround(filter.getFieldId().toLowerCase())).replace(value,
				varcharValue);
	}
