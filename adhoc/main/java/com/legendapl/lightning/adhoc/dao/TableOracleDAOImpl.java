...
public class TableOracleDAOImpl extends TableGenericDAO {

	private static final String maxLengthSqlFormat = "SELECT * FROM (SELECT \"fn\" FROM \"tn\" WHERE LENGTH(\"fn\") = (SELECT MAX(LENGTH(\"fn\")) FROM \"tn\")) WHERE ROWNUM <= 1";

	private static final String distinctSqlFormat = "SELECT DISTINCT \"fn\" FROM \"tn\"";

	private static final String distinctNumberSqlFormat = "SELECT * FROM (SELECT \"tn\".\"fn\" FROM \"tn\" ORDER BY \"tn\".\"fn\" DESC) WHERE ROWNUM <= 1";
	...
	private OracleUtils oracleUtils = new OracleUtils(this);

	public TableOracleDAOImpl(Adhoc adhoc) {
		super(adhoc);
		oracleUtils.OracleFilter();
	}

	public TableOracleDAOImpl(DataSourceImpl dataSource) {
		super(dataSource);
	}

	@Override
	protected void generateSQL() {
		oracleUtils.generateSelectStatement();
		generateFromStatement();
		generateWhereStatement();
		generateOrderStatement();
		sqlStatement = OracleUtils.replaceSql(sqlStatement);
	}

	private void generateOrderStatement() {
		...
		if (!orderStatement.trim().isEmpty()) {
			sqlStatement += "ORDER BY " + orderStatement.toUpperCase();
		}
	}
	...
	protected void generateAdhocWhereStatement() {
		if (adhoc.getFilterCheckFlg()) {
			adhocWhereStatement = OracleUtils.getFilterConnect(adhoc.getFilters(), adhoc.getFilterConnect().replaceAll(" +", " ").trim());
		} else {
			adhocWhereStatement = "";
		}
	}

	private void generateFromStatement() {
		if(adhoc.getTopicTree().getJoins().isEmpty()) {
			fromStatement = "FROM " + getRealTableStatement(adhoc.getTopicTree().getId()) + "\n";
		} else {
		...
		sqlStatement += fromStatement.toUpperCase();
	}
	...
	public String getMaxLength(Field field) {
		...
		sql = sql.replaceAll("tn", OracleUtils.replaceTableName(field.getTableName()));
	...
	public String getRealTableStatement(String tableId) {
		String tableName = adhoc.getDbTableIdToName().get(tableId.toUpperCase());
		if(tableName.equals(tableId))
			return SQLUtils.DBsurround(tableId);
		return OracleUtils.DBsurround(tableName) + " " + SQLUtils.DBsurround(tableId);
	}
