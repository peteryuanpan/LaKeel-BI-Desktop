...
public class TableSqlserverDAOImpl extends TableGenericDAO {

	private static final String maxLengthSqlFormat = "SELECT TOP 1 \"FN\" FROM \"TN\" WHERE LEN(\"FN\") = (SELECT MAX(LEN(\"FN\")) FROM \"TN\")";

	private static final String distinctSqlFormat = "SELECT DISTINCT \"FN\" FROM \"TN\"";

	private static final String distinctNumberSqlFormat = "SELECT TOP 1 \"TN\".\"FN\" FROM \"TN\" ORDER BY \"TN\".\"FN\" DESC";
	...
	public TableSqlserverDAOImpl(Adhoc adhoc) {
		super(adhoc);
		sqlserverUtils.sqlServerFilter();;
	}

	public TableSqlserverDAOImpl(DataSourceImpl dataSource) {
		super(dataSource);
	}
	...
	public String getMaxLength(Field field) {
		...
		sql = sql.replaceAll("TN", SqlserverUtils.replaceTableName(field.getTableName()));
